...
class Checkergame:
    ...
    def add_captured_piece(self, color):
        radius = 25
        x_offset = (self.red_captured if color == 'red' else self.blue_captured) % 2 * (radius + 5)
        y_position = (self.red_captured // 2 if color == 'red' else self.blue_captured // 2) * (radius + 5)
    ...
    def check_winner(self):
        red_pieces = [p for p, _, _ in self.pieces if self.canvas.itemcget(p, 'fill') == 'red']
        blue_pieces = [p for p, _, _ in self.pieces if self.canvas.itemcget(p, 'fill') == 'blue']

        if not red_pieces:
            self.turn_label.config(text="Blue Wins!")
            return True
        elif not blue_pieces:
            self.turn_label.config(text="Red Wins!")
            return True
